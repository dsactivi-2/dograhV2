...
```bash
cd dograh
curl -o update_remote.sh https://raw.githubusercontent.com/dsactivi-2/dograhV2/main/scripts/update_remote.sh
...
<CodeGroup>
```bash macOS/Linux
curl -o docker-compose.yaml https://raw.githubusercontent.com/dsactivi-2/dograhV2/main/docker-compose.yaml
curl -o start_docker.sh https://raw.githubusercontent.com/dsactivi-2/dograhV2/main/scripts/start_docker.sh && chmod +x start_docker.sh
docker compose down
./start_docker.sh
```
```powershell Windows
Invoke-WebRequest -OutFile docker-compose.yaml https://raw.githubusercontent.com/dsactivi-2/dograhV2/main/docker-compose.yaml; Invoke-WebRequest -OutFile start_docker.ps1 https://raw.githubusercontent.com/dsactivi-2/dograhV2/main/scripts/start_docker.ps1; docker compose down; .\start_docker.ps1
```
</CodeGroup>
...
<Warning>
Rolling back across a database migration is not always safe — if the newer release ran a schema migration, downgrading may leave the DB in a state the older API doesn't understand. If in doubt, [open an issue](https://github.com/dsactivi-2/dograhV2/issues) before rolling back.
</Warning>
...
For a fork deployed from source, use the included wrapper instead. It aborts when
tracked source files or local commits would be affected, creates a local rollback
tag, updates the selected branch, updates submodules, validates the remote
configuration, then rebuilds:

```bash
./scripts/update_source_build.sh

# Deploy a tested fork branch before it has been merged into main:
./scripts/update_source_build.sh --branch agent/deepgram-2-stt-profile
```

The wrapper never modifies `.env`, Docker volumes, or deployment secrets. It is
only for installations with `docker-compose.override.yaml` (source-build mode).
Do not use `update_remote.sh` for a source build; that script refreshes the
prebuilt-image deployment bundle instead.
