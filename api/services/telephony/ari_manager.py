...
class ARIManager:
    ...
    async def _load_ari_configs(self) -> list:
        ...
        for row in rows:
            ...
            # Missing ws_client_name means externalMedia audio cannot be set up.
            # Park the config immediately rather than connecting in a broken state.
            if not ws_client_name:
                ...
                if self._should_log_validation_failure(
                ...
                reason = f"{failure.code}: {failure.internal_message}"
                try:
                    await db_client.set_telephony_configuration_inactive(
                        row.id, row.organization_id, reason
                    )
                    logger.warning(
                        f"[ARI Manager] Parked config {row.id} for org {row.organization_id}: "
