...
class UserConfigurationValidator:
    def __init__(self):
        self._validator_map = {
            ...
            ServiceProviders.DEEPGRAM_3.value: self._check_deepgram_api_key,
    ...
    def _check_fish_audio_api_key(self, model: str, api_key: str) -> bool:
        # Best-effort smoke test against Fish Audio's model list. Only a clear
        # auth failure rejects the save; other statuses stay inconclusive.
        try:
            response = httpx.get(
        ...
        if response.status_code in (401, 403):
            raise ValueError(
                ...
                "active. You can find your keys at https://fish.audio/app/api-keys/."
            )
        return True
